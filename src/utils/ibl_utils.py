...
def load_keypoints(one, eid, camera='left',collection='alf'):
    # sess_loader = SessionLoader(one, eid=eid)
    dlc_data = load_dlc_data(one, eid, camera)['dlc']
    # filter out keypoints that contain likelihoods
    keypoints = {k: v for k, v in dlc_data.items() if 'likelihood' not in k}
    return keypoints
...
def prepare_data(one, eid, n_workers=os.cpu_count()):
...
def get_video_info(one, eid, camera,silent=True):
    urls = vidio.url_from_eid(eid, one=one)
    url = urls[camera]  # URL for the left camera
    meta = vidio.get_video_meta(url, one=one)
    for k, v in meta.items():
        print(f'The video {k} = {v}') if not silent else None
    return meta, url
...
def save_grayscale_video(video, save_path, fps=60):
    # save the video as grayscale
    h, w = video[0].shape[:2]
    fourcc = cv2.VideoWriter_fourcc(*'mp4v')
    out = cv2.VideoWriter(save_path, fourcc, fps, (w, h), 0)
    for frame in video:
        out.write(frame)
    out.release()

def save_grayscale_image(image, save_path):
    # save the image as grayscale
    # image shape is (height, width)
    # write the image using cv2
    cv2.imwrite(save_path, image)

def resize_video(video, width=640, height=480):
    return np.array([cv2.resize(frame, (width, height)) for frame in video])

def save_grayscale_video_image(video, save_dir, fps=60, width=640, height=480):
    # video directory
    ...
    fourcc = cv2.VideoWriter_fourcc(*'mp4v')
    out = cv2.VideoWriter(os.path.join(video_dir, 'video.mp4'), fourcc, fps, (width, height), 0)
    for i, frame in enumerate(video):
        _frame = cv2.resize(frame, (width, height))
        out.write(_frame)
        # save the image
        # cv2.imwrite(os.path.join(image_dir, f'{i}.png'), _frame)
        np.save(os.path.join(image_dir, f'{i}.npy'), _frame)
    out.release()